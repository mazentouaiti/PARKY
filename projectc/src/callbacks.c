                                        ...
                                        gpointer         user_data)
{
    ...
    int *aa = gtk_entry_get_text(GTK_ENTRY(numberofspaces));
    if (check_parking_id(bb,aa)== 0) {
...
GtkWidget *parent_window = GTK_WIDGET(gtk_widget_get_toplevel(GTK_WIDGET(refresh1)));
GtkWidget *ADID;
ADID = lookup_widget(parent_window, "ADID");

char idee[10];
sprintf(idee,"%d",logged_in_citizen_id);
// Display data in respective widgets
    GtkTextBuffer *buffer;


buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(ADID));
gtk_text_buffer_set_text(buffer, idee, -1);
...
GtkWidget *parent_window = GTK_WIDGET(gtk_widget_get_toplevel(GTK_WIDGET(refresh2)));

GtkWidget *ADID2;

ADID2 = lookup_widget(parent_window, "ADID2");
char idee[10];
sprintf(idee,"%d",logged_in_citizen_id);
// Display data in respective widgets
    GtkTextBuffer *buffer;
...
buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(ADID2));
gtk_text_buffer_set_text(buffer, idee, -1);
}
